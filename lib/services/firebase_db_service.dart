...
class FirebaseDBService {
  ...
  static List<DocumentSnapshot<Object?>> messages = const [];
    ...
    if (messages.isEmpty) {
      ...
      if (messages.last[FBAtt.msgDeleteFrom.name] == null) {
        ...
        if (List<String>.from(messages.last[FBAtt.msgDeleteFrom.name]).contains(otherUserID)) //check other person is conversion deleted or not if delete then insert again or update
        {
          insertConversationFirstTime(
             currentUserID,
             otherUserID,
  ...
  static Stream<QuerySnapshot> getMessages(String currentUserID, String otherUserID) {
    String childChatId = getChatId(currentUserID, otherUserID);
    return _firestore.collection('chats').doc(childChatId).collection('messages').orderBy('timestamp', descending: true).snapshots();
  }
  ...
  static updateOnlineStatus(String userID, bool status) {
    ...
    FirebaseFirestore.instance.runTransaction((transaction) async {
      transaction.update(
          FirebaseFirestore.instance.collection('user').doc(userID), userInfo);
    }).onError((error, stackTrace) {
  ...
  static Future<void> addedUser(User user, String userType) async {
